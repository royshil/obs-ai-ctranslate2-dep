...
if [[ ${ACCEL} = "nvidia" ]]; then
  ...
  additional_args="-DWITH_CUDA=OFF -DWITH_CUDNN=OFF -DWITH_TENSOR_PARALLEL=OFF -DWITH_HIP=ON -DCMAKE_C_COMPILER=hipcc -DCMAKE_CXX_COMPILER=hipcc \"-DCMAKE_HIP_ARCHITECTURES=${AMD_GPU_TARGETS}\" \"-DGPU_TARGETS=${AMD_GPU_TARGETS}\""
  extra_cxx_flags="-fPIC --no-warnings"
else
